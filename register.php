...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$login || !$password) {
        ...
        $sql = "SELECT id FROM CURS_user WHERE login = ?";
        $stmt = sqlsrv_query($conn, $sql, array($login));
        $user_exists = sqlsrv_fetch_array($stmt);
        sqlsrv_free_stmt($stmt);
        
        if ($user_exists) {
            $error = 'Логин уже существует';
        } else {
            // Хешируем пароль (не храним в открытом виде!)
            $hash = password_hash($password, PASSWORD_DEFAULT);
            
            // Добавляем нового пользователя
            $sql = "INSERT INTO CURS_user (login, password) VALUES (?, ?)";
            $stmt = sqlsrv_query($conn, $sql, array($login, $hash));
...
<head>
    ...
    <title>Регистрация | Кулинарный портал</title>
    <style>
        * {
            margin: 0;
            padding: 0;
