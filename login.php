...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($login && $password) {
        ...
        $sql = "SELECT * FROM CURS_user WHERE login = ?";
        $stmt = sqlsrv_query($conn, $sql, array($login));
        $user = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC);
        
        // Проверяем пароль (password_verify расшифровывает хеш)
        if ($user && password_verify($password, $user['password'])) {
            // Сохраняем данные пользователя в сессию
            $_SESSION['user_id'] = $user['id'];
            $_SESSION['user_login'] = $user['login'];
            header('Location: main.php');  // Редирект на главную
            exit;
        } else {
...
<head>
    ...
    <title>Вход | Кулинарный портал</title>
    <style>
        * {
            margin: 0;
            padding: 0;
